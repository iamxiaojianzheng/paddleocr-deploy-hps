...
document.addEventListener('DOMContentLoaded', () => {
    // 当前选中的 API
    let activeApi = 'layout-parsing';

    // 1. 初始化自动接力组件
    DataRelay.init();
    
    // 默认隐藏接力提醒
    DataRelay.updateAlertUI(false);

    // ==================== A. API Tab 切换交互 ====================
    const apiTabs = document.getElementById('apiTabs');
    const layoutForm = document.getElementById('layoutParsingForm');
    const restructureForm = document.getElementById('restructurePagesForm');

    apiTabs.addEventListener('click', (e) => {
        if (!e.target.classList.contains('tab-btn')) return;

        // 切换 Tab Class
        Array.from(apiTabs.children).forEach(btn => btn.classList.remove('active'));
        e.target.classList.add('active');

        // 切换对应的参数表单
        activeApi = e.target.getAttribute('data-tab');
        if (activeApi === 'layout-parsing') {
            layoutForm.classList.add('active');
            restructureForm.classList.remove('active');
        } else {
            layoutForm.classList.remove('active');
            restructureForm.classList.add('active');
            
            // 切换到多页重构时，如果有接力数据，自动填入
            if (DataRelay.lastParsedPages) {
                DataRelay.applyRelayData();
            }
        }
    });
    ...
    btnSend.addEventListener('click', async () => {
        ...
        try {
            if (activeApi === 'layout-parsing') {
                ...
                endpoint = '/restructure-pages';
                payload = ParamCollector.collectRestructurePagesParams();
            }
        } catch (err) {
        ...
        // 如果是版面解析接口请求成功，缓存结果，开启自动接力
        if (activeApi === 'layout-parsing' && response.status === 200) {
            DataRelay.cacheFromParsingResponse(response.data);
        }
    });
