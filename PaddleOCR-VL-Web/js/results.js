...
const ResultRenderer = {
    ...
    render(responseData, statusCode, duration) {
        ...
        if (resultsArray && resultsArray.length > 0) {
            ...
            // 没有版面解析结果（例如某些异常响应但 errorCode 为 0，极少见）
    ...
    renderError(data) {
        document.getElementById('pageSelector').style.display = 'none';
        document.getElementById('docxDownloadBar').style.display = 'none';
    ...
    /**
     * 渲染单页结果数据 (Markdown、可视化图像、导出 DOCX 等)
    ...
    renderPageData(pageData) {
        ...
        if (outputImages && Object.keys(outputImages).length > 0) {
            for (const [key, base64] of Object.entries(outputImages)) {
                if (base64) {
                    ...
                    titleEl.textContent = key; // 使用 textContent，绝对安全
        ...
        // 4. Word 导出结果处理 (exports.docx)
        const docxDownloadBar = document.getElementById('docxDownloadBar');
        const exports = pageData.exports || {};
        
        if (exports.docx && exports.docx.content) {
            docxDownloadBar.style.display = 'flex';
            ...
            newBtn.addEventListener('click', () => {
                ApiClient.downloadBase64File(
                    exports.docx.content, 
                    `paddleocr_layout_page_${this.currentPageIndex + 1}.docx`, 
                    'application/vnd.openxmlformats-officedocument.wordprocessingml.document'
                );
                showToast('DOCX 文件已开始下载');
            });
        } else {
            docxDownloadBar.style.display = 'none';
        }
    },
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (modal && modalImg) {
        ...
        modalImg.addEventListener('mousedown', (e) => {
            ...
            modalImg.style.transition = 'none'; // 拖动时临时移除过渡，保证完美的跟手响应
        });
