...
| `合并版面区域`                 | `mergeLayoutBlocks`         | `boolean` ｜ `null`                   | 是否合并相邻的同类型版面区域块。                             | 否       |
| `返回 Markdown 图片`           | `returnMarkdownImages`      | `boolean`                              | 是否在响应中返回 Markdown 中引用的图片。默认为 `true`；设为 `false` 时 `markdown.images` 为 `null` 或不出现，且服务端跳过图片编码 / URL 上传。 | 否       |
| `过滤标签`                     | `markdownIgnoreLabels`      | `array` ｜ `null`                      | 指定需要在输出的 Markdown 文本中过滤掉的标签列表，输入JSON数组，例如 `["header", "footer"]`。 | 否       |